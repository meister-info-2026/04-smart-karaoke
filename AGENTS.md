# AGENTS.md — [agent-vibe-coding-starter-kit2-karaoke]

## 프로젝트 개요

- 팀 주제: `웹 예약 연동 자동화 학교 노래방 부스 관리 시스템`
- 한 줄 목표: `웹사이트 예약 및 일회성 비밀번호 인증을 통해 부스 전원, 도어락, 오디오 및 안내 시스템을 자동 제어하는 무인 자동화 학교 노래방 시스템 구축`
...
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 팀 주제                    | `웹 예약 연동 자동화 학교 노래방 부스 관리 시스템`                                                                                                                                                                                                                                                                                                                                                                                                             |
| 액추에이터(제어 대상) 목록 | `솔레노이드 도어락, 릴레이 모듈(기기 전원), LED 모듈, 스피커/오디오 모듈`                                                                                                                                                                                                                                                                                                                                                                                      |
| 센서(모니터링 대상) 목록   | `4x4 Keypad(비밀번호 입력), 감지 센서(입장 감지)`                                                                                                                                                                                                                                                                                                                                                                                                              |
| 영상인식 감지 대상         | `없음`                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 트리거 규칙                | `예약된 이용 시간에 따라 부스 전원과 장치를 자동 제어한다. 일회성 4자리 비밀번호 인증 성공 시 도어락을 해제하고 릴레이를 통해 전원을 공급한다. 관리자 고정 비밀번호(9179) 입력 시 노래방 기기 전원은 제외하고 도어락을 해제하며 LED만 점등한다. 입장 시 환영 안내를 출력하고, 이용 종료 10분 전에 LED를 깜빡여 알리며, 이용 종료 시 지정된 종료 음악을 재생한 후 전원과 마이크를 차단한다. 동일 요일/시간대 중복 예약은 차단하고 당일 예약은 허용하지 않는다.` |
| 팀원 역할 분담             | `백승환 → hardware·라즈베리파이(조장) / 조민규 → hardware·기구물 / 김민제 → backend·db·frontend / 박민성 → backend·db·frontend`                                                                                                                                                                                                                                                                                                                                |
