...
- **RP1 칩셋 구조 특성**: 라즈베리파이 5(Debian Bookworm/Trixie)는 자체 I/O 컨트롤러인 RP1 칩셋을 사용하여 기존 `RPi.GPIO`가 작동하지 않습니다. 반드시 **`gpiozero`** 라이브러리와 **`lgpio`** 백엔드를 사용해야 합니다.
- **pip 빌드 오류 해결**: PyPI에서 `pip install lgpio` 실행 시 C 헤더/컴파일 도구 부재로 인해 컴파일 에러가 발생합니다. 따라서 라즈베리파이 OS 기본 APT 패키지(`python3-gpiozero`, `python3-lgpio`)를 사용하도록 표준화합니다.
- **가상환경 연동**: `python3 -m venv --system-site-packages venv`로 가상환경을 생성하여 시스템에 설치된 `gpiozero`와 `lgpio`를 venv 내부에서 그대로 재사용합니다.

### ③ 라즈베리파이 환경 설정 및 데몬 실행 방법
1. 라즈베리파이와 Windows PC를 동일한 공유기(와이파이)에 연결합니다.
2. Windows PC의 IP 주소를 확인합니다 (PowerShell에서 `ipconfig` 실행, 예: `192.168.0.25`).
3. **패키지 설치 및 가상환경 생성**:
   ```bash
