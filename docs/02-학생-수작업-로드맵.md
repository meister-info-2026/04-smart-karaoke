...
- [ ] `backend/.env`의 **`ADMIN_PIN`** 확인 (기본 `9179` — 전시 전에는 반드시 다른 값으로 변경)
- [ ] `backend/.env`의 **`ALLOW_TEST_PIN`** 확인 (로컬 개발은 `true`, **전시 때는 반드시 `false`**)
- [ ] `frontend/.env`의 **`NEXT_PUBLIC_API_BASE_URL` / `NEXT_PUBLIC_WS_URL`** 이 백엔드 주소와 같은지 확인
...
- [ ] DB 초기화 스크립트 실행 (테이블 7개 + 디바이스·애창곡 시드 데이터 생성):
      *이미 DB를 만들어 둔 팀도 한 번 더 실행하세요 — `song_videos` 테이블이 나중에 추가되었습니다*
  ```powershell
  cd backend
...
- [ ] **1차 완성 핵심 기능 직접 조작 검증**:
  ...
  - [ ] **같은 PIN 재입력 ➔ "이미 사용한 비밀번호"로 거부되는지 확인** (PIN은 1회용)
  - [ ] **관리자 인증**: 첫 탭 상단 상자에 `.env`의 `ADMIN_PIN` 입력 ➔ 기기 제어 버튼이 활성화되는지 확인
  - [ ] 키패드에 관리자 PIN 입력 ➔ 도어락 UNLOCKED + LED ON (릴레이 전원 OFF 유지) 확인
  - [ ] 웹캠 사람 감지 시 대시보드 환영 안내(TTS) 자동 송출 확인
  - [ ] **노래방 [🎤 입장하기] ➔ 마이크 권한 허용 ➔ 주변 소음 측정 완료 메시지 확인**
  - [ ] **애창곡 10곡을 하나씩 눌러 유튜브 노래방 영상이 실제로 재생되는지 확인**
        *(안 나오는 곡은 화면에서 다른 링크를 등록 — 관리자 인증 필요)*
  - [ ] 노래를 부르면 음정/성량이 실시간 반응하고, **곡이 끝나면 점수가 자동으로 뜨는지** 확인
