...
> 📂 **부록A / 백엔드·DB 담당 + 라즈베리파이 담당 / 2차 개발(하드웨어 연동)** · 전체 목록 [docs/README.md](README.md)

> **언제 이 문서를 보나요?** `docs/01-학생용-설치-및-사용-매뉴얼.md`의 **라즈베리파이 5
> 하드웨어 마이그레이션** 단계에서, 라즈베리파이 5 관련 개발(새 액추에이터 추가, 폴링
...
sequenceDiagram
    ...
    FE->>BE: POST /api/devices/door_lock_1/control (X-Admin-Token, desired_state)
...
| `POST /api/devices/{device_id}/control` | 대시보드(**관리자**) | `X-Admin-Token` | 참고용 — 관리자가 desired-state를 만드는 진입점. 관리자 PIN으로 `POST /api/admin/login`에서 토큰을 먼저 받는다 (부록G §3-4) |
...
전체 목록은 `docs/부록C-iot-test-system-연동-가이드.md` 3장.
...
docs/부록A-백엔드-라즈베리파이5-연동-인터페이스-가이드.md의 3장 REST 계약(JSON 스펙 그대로)에 맞춰
...
docs/부록A-백엔드-라즈베리파이5-연동-인터페이스-가이드.md의 3장 REST 계약대로 pi/main.py의
...
| gpiozero `BadPinFactory` 또는 `pip install lgpio` C 컴파일 에러 | Pi 5(RP1 칩) 특성 및 PyPI C 소스 컴파일 도구 부재 | 1) pip 대신 `sudo apt install -y python3-gpiozero python3-lgpio` 실행<br>2) `python3 -m venv --system-site-packages venv`로 가상환경 생성 후 패키지 상속<br>3) `pi/.env`의 `GPIOZERO_PIN_FACTORY=lgpio` 확인 |
...
- **`docs/부록C-iot-test-system-연동-가이드.md`** — 라즈베리파이 담당자가 임시 테스트
